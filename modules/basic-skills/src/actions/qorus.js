...
const callApi = async (url, method, body, memory, variable, headers) => {
  ...
  try {
    ...
    event.state[memory][variable] = {
      body: data,
      json: JSON.stringify(data), // this is here to make it easier to debug in the botpress studio
    ...
    event.state[memory][variable] = { status: errorCode, json: JSON.stringify(error) }
