...
import ConnectorField, { t } from './dataProvider'
...
const AutoField: FunctionComponent<any> = ({
  ...
  const renderField = (currentType: string) => {
    ...
    switch (currentType) {
      ...
      case 'date':
        return <DateField fill {...rest} name={name} onChange={handleChange} value={value} type={currentType} />
      case 'hash':
      case 'hash<auto>':
      case 'list':
      ...
      // }
      case 'data-provider': {
        return <ConnectorField value={value} name={name} inline minimal isConfigItem onChange={handleChange} />
      }
      // case 'file-as-string': {
      //   return (
      //     <div>
  ...
  const showPicker =
    !isSetToNull && (defaultType === 'auto' || defaultType === 'any' || currentType === 'auto' || currentType === 'any')
