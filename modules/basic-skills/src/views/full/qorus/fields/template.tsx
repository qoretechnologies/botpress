...
import { ReqoreControlGroup, ReqoreMessage, ReqoreTabs, ReqoreTabsContent, ReqoreTag } from '@qoretechnologies/reqore'
  ...
  return (
    ...
    >
      ...
      <ReqoreTabsContent tabId={'template'}>
        <ReqoreMessage size="small" intent="info" inverted>{`${t(
          'Templates are in format: '
        )} $<type>:<key>`}</ReqoreMessage>
        <Spacer size={5} />
        <ReqoreControlGroup fluid stack>
          <ReqoreTag label="$" color="#d7d7d7" style={{ borderTopRightRadius: 0, borderBottomRightRadius: 0 }} />
          <Select
...
const PreviewTemplate = (props: any) => {
  ...
  return <TemplateField component={AutoField} {...props} value={value} onChange={(_name, val) => setValue(val)} />
}
